function [net,trainInfo] = AccuSleep_train_org(fileList, SR, epochLen, epochs, imageLocation)
...
%imageLocation = '/Volumes/T9/accusleep_data/prep_balanced/training_images_2024-01-19_12-07-21/'; 
...
% 
% disp('Training network')
% %N = sum([11100,38041,58619]);
% %weights = [N/(3*11100),N/(3*38041),N/(3*58619)];
% %customLossFunction = @(Y, T) crossentropy(Y, T,weights);
% N = sum([11100,38041,58619]);
% weights = dlarray([N/(3*11100), N/(3*38041), N/(3*58619)],'C');
% customLossFunction = @(Y, T) crossentropy(Y, T,weights);
